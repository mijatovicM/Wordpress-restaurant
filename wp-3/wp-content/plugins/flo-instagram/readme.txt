...
<ul>
	...
	<li><b>Shortcode</b> - add your feed to any page or post using a shortcode. Also, include an Instagram follow button.</li>
...
1. Upload the `flo-instaram` directory to your `/wp-content/plugins/` directory
2. Activate the plugin through the 'Plugins' menu in WordPress
3. Visit the 'FLO Social' menu item in your admin sidebar to enter your Instagram user ID and the Access Token. To generate the access token, the plugin provides a link to our server which will redirect you to http://instagram.com to authenticate and authorize our application. Once you authorize the app, it will generate the access token which will be shown on the screen. We do not store any user data on our end, the only reason of sending the user to our site is that Instagram requires the client secrete key, which we can not make public.
