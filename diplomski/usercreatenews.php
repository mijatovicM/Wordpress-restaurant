...
if(!isset($_SESSION['userId']) || $_SESSION['userType'] == 'urednik' || $_SESSION['userType'] == 'redakcija') {
...
<div>
    ...
    if(isset($_POST['editor']) && isset($_POST['title']) && isset($_POST['caption']) && isset($_POST['submit']) && isset($_POST['alt']) && isset($_POST['newstype']) && isset($_POST['hashtags']) && isset($_SESSION['userId'])) {
        ...
        if(in_array($imageActualExt, $allowed)){
            if($imageError===0){
                if($imageSize<10000000 ){
                    ...
                    exit('<script>location.replace("usercreatenews.php?error=imagesize");</script>');


                }
            }else{

                exit ('<script>location.replace("usercreatenews.php?error=imageerror");</script>');
            }
        }
        else{
            exit ('<script>location.replace("usercreatenews.php?error=badtype");</script>');
        }
        ...
        if (!$error) {
            ...
            $query = mysqli_query($connection, "INSERT INTO news (title,caption,images,alt,timeofinsert,newstype,content,important_news,approved,creator,creatorname,cookie_count) VALUES ('$title','$caption','$output_dir$imageNameNew','$alt',NOW(),'$newstype','$text',0,0,'korisnik','$creatorname',0 )");

            if ($query) {
                $last_id = mysqli_insert_id($connection);
                echo  '<script>location.replace("usercreatenews.php?success=newssuccess");</script>';
            } else {
                echo  '<script>location.replace("usercreatenews.php?error=errornews");</script>';
