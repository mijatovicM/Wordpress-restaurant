...
<div>
    ...
    echo "<div class='hashtag' style='width: 80%;border-radius: 7px 7px;margin: 4% auto 0 auto;'>Vesti naših korisnika</div>";

    //defining how many results we want per page
    $results_per_page=6;
    $sql = "SELECT * FROM news WHERE creator='korisnik' AND approved=1  ORDER BY id DESC";
    /*
    ...
    $sql = "SELECT * FROM news WHERE creator='korisnik' AND approved=1 ORDER BY id DESC LIMIT ".$this_page_first_result.','.$results_per_page;
    $result = mysqli_query($connection, $sql);
    $number_of_results=mysqli_num_rows($result);
    if (mysqli_num_rows($result) > 0) {
        ...
        echo "<div class='hashtag'>Nema rezultata</div>";
    ...
    for($i=1;$i<=$number_of_pages;$i++){
        ...
        if($i==$page)
            ...
            echo '<a href="usernews.php?page='.$i.'">'.$i.'</a>';
    }
    echo '</ul>';
    echo '</div>';
    ?>
