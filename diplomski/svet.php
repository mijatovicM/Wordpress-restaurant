...
<div>
    ...
    <div class="cardscontainer" style='margin: 2%;'>
        <div class="row">
            <?php
            $sql = "SELECT * FROM news WHERE newstype='svet' AND approved=1  ORDER BY id DESC LIMIT 0,4 ";
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while($row = mysqli_fetch_assoc($result)) {
                    ...
                    $id=$row["id"];
                    echo '<a class="btn btn-secondary buttonMore" 
                     href="news.php?id='.$id.'">Pročitajte više..</a>';
                    echo"</div>";
                    echo "</div>";
                }
            } else {
...
?>
            ...
            $results_per_page=6;
            $sql = "SELECT * FROM news WHERE newstype='svet' AND approved=1  ORDER BY id DESC";
            $result = mysqli_query($connection, $sql);
            ...
            $sql = "SELECT * FROM news  WHERE newstype='svet' AND approved=1  ORDER BY id DESC LIMIT " .$this_page_first_result.','.$results_per_page;
            $result = mysqli_query($connection, $sql);
            $number_of_results=mysqli_num_rows($result);
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
            ...
            if(isset($_GET['page'])) {
                if ($_GET['page'] > 1) {
                    echo "<li><a href='svet.php?page=" . ($page - 1) . "'style='font-size: 25px;padding-top: 3%;'  class='button'><i class=\"fas fa-chevron-circle-left\"></i></a></li>";
                }}
            else echo "";
            if(!isset($_GET['page'])){
                echo "";
            ...
            if($page<=2 ){
                for($i=1;$i<=2;$i++){
                    ...
                    if($i==$page)
                        ...
                        echo '<a href="svet.php?page='.$i.'">'.$i.'</a>';
                }
            }
            if($page>2 ) {
                echo '<a class="active" href="svet.php?page=' . $page . '">' . $page . '</a>';
            }
            if(isset($_GET['page'])) {
                if ($_GET['page'] < $number_of_pages) {
                    echo "<li><a href='svet.php?page=" . ($page + 1) . "' style='font-size: 25px;padding-top: 3%;' class='button'><i   class=\"fas fa-chevron-circle-right\"></i></a></li>";
                }}
