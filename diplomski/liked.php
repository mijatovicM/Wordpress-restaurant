...
FROM news INNER JOIN liked_news ON news.id=liked_news.id WHERE users_id='$userid'  ORDER BY liked_news_id DESC ";
    $result = mysqli_query($connection, $sql);
    $number_of_results=mysqli_num_rows($result);
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            echo "<div class='cardsection' style='overflow: hidden;'>";
            ...
            echo "<br/><br/>";
            echo "<button class='btn btn-danger' onClick='dellike($id)' name='Delete'>Uklonite svoj lajk <i class=\"fas fa-eraser\"></i></button>";
            deletelike();
            echo "</div>";
            echo "</div>";
        ...
        echo "<div class='commentedh1' style='width: 80%;border-radius: 7px 7px;margin: 15% auto 0 auto;font-size: 40px'>Niste lajkovali nijednu vest</div>";
        echo "</div>";
    ...
    function deletelike(){?>
        ...
        <script language="javascript">
            function dellike(dellikeid)
            {
                if(confirm("Da li sigurno želite da uklonite svoj lajk?")){
                    window.location.href='deleteliked.php?dellike_id=' +dellikeid+'';
                    alert('Uspešno ste uklonili svoj lajk');
                    return true;
                }
            }
